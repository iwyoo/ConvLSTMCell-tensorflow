...
class ConvLSTMCell(rnn_cell.RNNCell):
  ...
  def __call__(self, inputs, state, scope=None):
    ...
    with vs.variable_scope(scope or type(self).__name__): # "ConvLSTMCell"
      ...
      concat = self._conv([inputs, h], 4 * self,_num_units, 3, True)

      # i = input_gate, j = new_input, f = forget_gate, o = output_gate
      i, j, f, o = array_ops.split(4, 4, concat)
  ...
  def _conv(args, output_size, strides=1, bias=True, bias_start=0.0, scope=None):
    ...
    for shape in shapes:
      ...
      if not shape[3]:
        raise ValueError("Conv expects shape[2] of arguments: %s" % str(shapes))
    ...
    with vs.variable_scope(scope or "Conv"):
      ...
      if len(args) == 1:
        res = tf.nn.conv2d(args[0], kernel, strides=[1, strides, strides, 1], padding='SAME')
      else:
        res = tf.nn.conv2d(array_ops.concat(3, args), kernel, strides=[1, strides, strides, 1], padding='SAME')
